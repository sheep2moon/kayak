import { addDays, addMonths, getMonth, getYear, subMonths } from 'date-fns';
...
const Calendar = ({ pickDate }) => {
  ...
  const backwardMonth = () => {
    const monthBack = subMonths(currentMonth, 1);
    if (addDays(monthBack, 1) > new Date()) {
      setCurrentMonth(monthBack);
    }
  };
